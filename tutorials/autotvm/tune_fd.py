...
def load_tf_model():
    ...
    with tf_compat_v1.gfile.GFile(model_path, 'rb') as graphfile:
        ...
        image_tensor, box, score  = tf_compat_v1.import_graph_def(graph_def, name='',return_elements=[
                 'Preprocessor/map/TensorArrayStack/TensorArrayGatherV3','Postprocessor/raw_box_encodings', 'Postprocessor/convert_scores'])

        # tf.io.write_graph(graph_def, '/tmp/mymodel', 'dump_face.pbtext')
        graph_def = tf_testing.ProcessGraphDefParam(graph_def)
        with tf_compat_v1.Session() as sess:
            shaped_graph_def = tf_testing.AddShapesToGraphDef(sess, ['Postprocessor/raw_box_encodings', 'Postprocessor/convert_scores'])

    print("\n\n ------------- Tensorflow graph defined -------------------- \n\n ")

    image_name = "image1.jpg"
    image_file = os.path.join(os.path.dirname(__file__), image_name)
    url = "https://previews.123rf.com/images/auremar/auremar1905/auremar190501096/123909218-mixed-couple-taking-a-selfie-outdoors.jpg"
    urllib.request.urlretrieve(url, image_file)

    image = cv2.imread(image_file)
    image = image[0:300, 0:300, 0:3]
    ...
    shape_dict = {'image_tensor': image_data.shape}
    layout = None
    output_names = ['Postprocessor/raw_box_encodings', 'Postprocessor/convert_scores']
    ...
    return mod, params, image_data.shape
...
def get_network(name, batch_size):
    ...
    if "resnet" in name:
        ...
        #output_shape = 
        mod, params, input_shape = load_tf_model()
    else:
...
def tune_and_evaluate(tuning_opt):
    ...
    with autotvm.apply_history_best(log_file):
        ...
        module.set_input('Preprocessor/map/TensorArrayStack/TensorArrayGatherV3', data_tvm)
